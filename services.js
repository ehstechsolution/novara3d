...
function renderProductCard(product, whatsappNumber, colorMap = {}) {
  ...
  const coresJson = cores.length ? JSON.stringify(cores).replace(/"/g, '&quot;') : '[]';

  const colorsHTML = cores.length
    ? `<div class="product-card__colors">${cores.map(hex => `<span class="color-dot" style="background:${hex}" title="${colorMap[hex] || ''}"></span>`).join('')}</div>`
    : '';

  return `
    <div class="product-card" data-id="${product.id}" data-fotos='${fotosJson}' data-titulo="${titulo.replace(/"/g, '&quot;')}" data-descricao="${descricao.replace(/"/g, '&quot;')}" data-preco="${preco != null ? preco : ''}" data-cores='${coresJson}' data-walink="${waLink}">
      ...
      <div class="product-card__body">
        ...
        ${preco != null ? `<div class="product-card__price">R$ ${preco.toFixed(2).replace('.', ',')}</div>` : ''}
        ${colorsHTML}
